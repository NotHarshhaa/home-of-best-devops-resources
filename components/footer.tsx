"use client";
import React from "react";
import Link from "next/link";
import { cn } from "@/lib/utils";
import { Button } from "@/components/ui/button";

const Footer = () => {
  return (
    <footer className="border-t bg-background">
      <div className="container py-8 md:py-12">
        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-4">
          <div className="flex flex-col space-y-2">
            <h3 className="text-lg font-bold">Home of Best DevOps Resources</h3>
            <p className="text-sm text-muted-foreground">
              Your Ultimate Collection of DevOps Learning Materials, Projects,
              and Tools for accelerating your DevOps journey.
            </p>
          </div>
          <div>
            <h4 className="mb-4 text-sm font-bold uppercase">Quick Links</h4>
            <ul className="space-y-2 text-sm">
              <li>
                <Link
                  href="/"
                  className="transition-colors hover:text-foreground/80"
                >
                  Home
                </Link>
              </li>
              <li>
                <Link
                  href="/resources"
                  className="transition-colors hover:text-foreground/80"
                >
                  Resources
                </Link>
              </li>
              <li>
                <Link
                  href="/categories"
                  className="transition-colors hover:text-foreground/80"
                >
                  Categories
                </Link>
              </li>
              <li>
                <Link
                  href="/about"
                  className="transition-colors hover:text-foreground/80"
                >
                  About
                </Link>
              </li>
            </ul>
          </div>
          <div>
            <h4 className="mb-4 text-sm font-bold uppercase">Categories</h4>
            <ul className="space-y-2 text-sm">
              <li>
                <Link
                  href="/categories/orchestration"
                  className="transition-colors hover:text-foreground/80"
                >
                  Orchestration
                </Link>
              </li>
              <li>
                <Link
                  href="/categories/ci-cd"
                  className="transition-colors hover:text-foreground/80"
                >
                  CI/CD
                </Link>
              </li>
              <li>
                <Link
                  href="/categories/infrastructure-as-code"
                  className="transition-colors hover:text-foreground/80"
                >
                  Infrastructure as Code
                </Link>
              </li>
              <li>
                <Link
                  href="/categories/monitoring"
                  className="transition-colors hover:text-foreground/80"
                >
                  Monitoring
                </Link>
              </li>
            </ul>
          </div>
          <div>
            <h4 className="mb-4 text-sm font-bold uppercase">Community</h4>
            <div className="flex space-x-4">
              <Button asChild variant="ghost" size="icon">
                <a
                  href="https://github.com/prodevopsguy/home-of-best-devops-resources"
                  target="_blank"
                  rel="noopener noreferrer"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="20"
                    height="20"
                    viewBox="0 0 24 24"
                    fill="none"
                    stroke="currentColor"
                    strokeWidth="2"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    className="h-5 w-5"
                  >
                    <path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.403 5.403 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65-.17.6-.22 1.23-.15 1.85v4" />
                    <path d="M9 18c-4.51 2-5-2-7-2" />
                  </svg>
                  <span className="sr-only">GitHub</span>
                </a>
              </Button>
              <Button asChild variant="ghost" size="icon">
                <a
                  href="https://twitter.com/prodevopsguy"
                  target="_blank"
                  rel="noopener noreferrer"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="20"
                    height="20"
                    viewBox="0 0 24 24"
                    fill="none"
                    stroke="currentColor"
                    strokeWidth="2"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    className="h-5 w-5"
                  >
                    <path d="M22 4s-.7 2.1-2 3.4c1.6 10-9.4 17.3-18 11.6 2.2.1 4.4-.6 6-2C3 15.5.5 9.6 3 5c2.2 2.6 5.6 4.1 9 4-.9-4.2 4-6.6 7-3.8 1.1 0 3-1.2 3-1.2z" />
                  </svg>
                  <span className="sr-only">Twitter</span>
                </a>
              </Button>
              <Button asChild variant="ghost" size="icon">
                <a
                  href="https://linkedin.com/in/prodevopsguy"
                  target="_blank"
                  rel="noopener noreferrer"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="20"
                    height="20"
                    viewBox="0 0 24 24"
                    fill="none"
                    stroke="currentColor"
                    strokeWidth="2"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    className="h-5 w-5"
                  >
                    <path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z" />
                    <rect width="4" height="12" x="2" y="9" />
                    <circle cx="4" cy="4" r="2" />
                  </svg>
                  <span className="sr-only">LinkedIn</span>
                </a>
              </Button>
            </div>
          </div>
        </div>
        <div className="mt-8 border-t pt-8 text-center text-sm text-muted-foreground">
          <p>
            © 2024 ProDevOpsGuy | Home of Best DevOps Resources. All rights
            reserved.
          </p>
        </div>
      </div>
    </footer>
  );
};

export default Footer;
