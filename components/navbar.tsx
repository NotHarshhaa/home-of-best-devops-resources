      ...
      <div className="container flex h-16 items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          >
            ...
            <div className="flex flex-col">
              <span className="hidden text-lg font-bold tracking-tight hover:text-primary/90 sm:inline-block transition-colors duration-200">
        ...
        <nav className="hidden md:flex items-center gap-8">
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="relative text-sm font-medium transition-colors hover:text-primary after:absolute after:bottom-[-2px] after:left-0 after:h-[2px] after:w-0 after:bg-primary after:transition-all hover:after:w-full"
            >
              {link.label}
            </Link>
          ))}
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="flex h-10 w-10 items-center justify-center rounded-md transition-colors hover:bg-primary/10 md:hidden"
          ...
          >
            <svg
              ...
              strokeLinecap="round"
              strokeLinejoin="round"
              className={`h-6 w-6 transition-all duration-300 ${mobileMenuOpen ? "rotate-90 scale-110" : ""}`}
            >
              {mobileMenuOpen ? (
                <>
            ...
            <div className="container py-4 flex flex-col space-y-4 bg-background">
              {navLinks.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className="text-foreground/80 hover:text-primary px-2 py-2 rounded-md hover:bg-primary/5 transition-colors"
